...
for variable in TARGET_RUN_DIR TARGET_RENDER_DIR TARGET_PIPELINE_ENV_FILE MOCK_DB XDS_DATABASE_NAME XDS_DATABASE_PORT XDS_DATABASE_USERNAME XDS_DATABASE_PASSWORD LMCACHE_L2_ENABLED LMCACHE_L2_BASE_PATH LMCACHE_L2_MAX_CAPACITY_GB LMCACHE_L2_NUM_WORKERS; do
  grep -qw "$variable" <<<"$local_writer" || {
    echo "$variable missing from local pipeline environment" >&2
    exit 1
  }
