...
# LMCache listens on 127.0.0.1 inside each Prefill Pod.  The named HTTP port
# follows the per-group port calculation in raycluster-cluster.yaml, so the
# probes never need to duplicate or guess an ordinal-specific port number.
lmcache_sidecar = values.get("lmcacheSidecar")
if lmcache_sidecar is not None and not isinstance(lmcache_sidecar, dict):
    raise SystemExit("lmcacheSidecar must be a mapping")
if isinstance(lmcache_sidecar, dict):
    lmcache_probe_defaults = {
        "startupProbe": {
            "httpGet": {
                "host": "127.0.0.1",
                "path": "/healthcheck",
    ...
    values["lmcacheSidecar"] = deep_merge(lmcache_probe_defaults, lmcache_sidecar)
    lmcache_sidecar = values["lmcacheSidecar"]

    if lmcache_sidecar.get("enabled", False):
        template_path = Path(chart_dir) / "templates" / "raycluster-cluster.yaml"
        if not template_path.is_file():
            raise SystemExit(f"LMCache sidecar template not found: {template_path}")
        template_text = template_path.read_text(encoding="utf-8")
        probe_marker = "{{- if $lmcache.startupProbe }}"
        if probe_marker not in template_text:
            resource_pattern = re.compile(
                r"(?m)^(?P<indent>[ \t]*)resources: \{\{- toYaml \$lmcache\.resources \| nindent (?P<nindent>\d+) \}\}\s*$"
            )
            match = resource_pattern.search(template_text)
            if match is None:
                raise SystemExit(
                    f"LMCache sidecar resources marker not found: {template_path}"
                )
            indent = match.group("indent")
            nindent = match.group("nindent")
            probes = "".join(
                f"{{{{- if $lmcache.{name} }}}}\\n"
                f"{indent}{name}:\\n"
                f"{{{{- toYaml $lmcache.{name} | nindent {nindent} }}}}\\n"
                f"{{{{- end }}}}\\n"
                for name in ("startupProbe", "readinessProbe", "livenessProbe")
            )
            template_text = template_text[:match.start()] + probes + template_text[match.start():]
