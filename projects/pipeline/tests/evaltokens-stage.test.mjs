...
/* 跨 vm realm 的对象不能用 deepStrictEqual（原型不同），统一走 JSON 序列化比较 */
...
test('EvalTokens 阶段配置持久化用户设置的参数覆盖值（values）', () => {
  ...
  jsonEqual(cfg.values, { model: 'qwen3', batch: '8' })
  jsonEqual(context.evaltokensStageConfig(null).values, {})
  ...
  jsonEqual(stage.evaltokens.values, { model: 'qwen3' })
  jsonEqual(stage.evaltokens.params, [{ key: 'model', label: 'model', def: 'qwen', required: false }], '编辑器内识别到的 params 仍为瞬态保留')
})
...
test('commitEvaltokParamValue：设置新值写入 values，清空或改回任务原值即删除', () => {
  ...
  jsonEqual(bare.values, { a: '1' }, 'values 缺失时自动创建')
})
...
test('EvalTokens 重新识别参数刷新定义与任务原值，已设置的值保留不刷新', async () => {
  ...
  jsonEqual(stage.evaltokens.params.map(p => [p.key, p.def]), [['model', 'qwen2.5'], ['batch', '8']], '参数定义与原值已刷新')
  jsonEqual(stage.evaltokens.values, { model: 'qwen3' }, '已设置的参数值不被识别结果刷新')
...
test('EvalTokens 识别不到任务时清空参数定义，但已设置的值仍保留', async () => {
  ...
  await context.detectEvaltokStageParams(0, false)
  assert.equal(stage.evaltokens.params.length, 0)
  jsonEqual(stage.evaltokens.values, { model: 'qwen3' })
