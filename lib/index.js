...
// src/index.ts
var PLUGIN_VERSION = false ? "dev" : "1.1.7";
var VERSION_HISTORY_CAP = 200;
function sanitizeVersionHistory(data) {
  const list = Array.isArray(data) ? data : data && typeof data === "object" && Array.isArray(data.history) ? data.history : [];
  const out = [];
  for (const e of list) {
...
function recordVersionInstall(history, version, now) {
  if (!/^\d+\.\d+\.\d+$/.test(version)) return null;
  const last = history[history.length - 1];
  if (last && last.version === version) return null;
  return [...history, { version, at: now }].slice(-VERSION_HISTORY_CAP);
}
var USAGE_KIND_RE = /^[a-z][a-z0-9_-]{0,31}$/;
function sanitizeUsageEvent(body, now) {
...
function aggregateUsageEvents(events, now) {
  ...
  for (const e of list) {
    const u = byUser.get(e.user) ?? { user: e.user, events: 0, visits: 0, opens: 0, days: /* @__PURE__ */ new Set(), firstAt: e.at, lastAt: e.at };
    u.events += 1;
    ...
    const d = byDay.get(day) ?? { events: 0, users: /* @__PURE__ */ new Set() };
    d.events += 1;
    d.users.add(e.user);
    byDay.set(day, d);
  }
  ...
  const users = [...byUser.values()].map((u) => ({ user: u.user, events: u.events, visits: u.visits, opens: u.opens, days: u.days.size, firstAt: u.firstAt, lastAt: u.lastAt })).sort((a, b) => b.events - a.events || b.lastAt - a.lastAt);
...
var DEFAULT_PIPELINE_SCRIPTS_DIR = pathResolve(PLUGIN_DIR, "projects", "pipeline", "scripts");
function resolvePipelineScriptsDir(config, installedDir) {
...
function cleanPipelineHistory(history) {
  const transient = /* @__PURE__ */ new Set(["_lc", "_lm", "_ll", "_profChecked", "_profState"]);
...
function mergePipelineConfigForWrite(clientConfig, baseConfig, diskConfig) {
  const objectConfig = (value) => value && typeof value === "object" && !Array.isArray(value) ? value : {};
  ...
  const valid = (item) => item && typeof item === "object" && !Array.isArray(item) && typeof item.id === "string" && item.id;
  const mapOf = (items) => new Map(items.filter(valid).map((item) => [item.id, item]));
  const clientMap = mapOf(clientPipelines), baseMap = mapOf(basePipelines), diskMap = mapOf(diskPipelines);
  const sameEntry = (left, right, id) => {
  ...
  const ids = /* @__PURE__ */ new Set([...baseMap.keys(), ...clientMap.keys(), ...diskMap.keys()]);
  ...
  const ordered = [];
  for (const item of diskPipelines) if (valid(item) && merged.has(item.id)) {
    ordered.push(merged.get(item.id));
    merged.delete(item.id);
  }
  for (const item of clientPipelines) if (valid(item) && merged.has(item.id)) {
    ordered.push(merged.get(item.id));
    merged.delete(item.id);
  }
...
function mergePipelineOneForWrite(clientPipeline, basePipeline, diskConfig) {
  ...
  const diskOne = diskPipelines.find((p) => p && typeof p === "object" && !Array.isArray(p) && p.id === id) ?? null;
  ...
  const config = { ...disk };
  if (diskOne) config.pipelines = diskPipelines.map((p) => p && p.id === id ? clientPipeline : p);
  else config.pipelines = diskPipelines.concat([clientPipeline]);
  return { conflicts: [], config };
}
function pipelineConfigDifferenceIds(clientConfig, diskConfig) {
  const pipelinesOf = (value) => value && typeof value === "object" && !Array.isArray(value) && Array.isArray(value.pipelines) ? value.pipelines : [];
  const mapOf = (items) => new Map(items.filter((item) => item && typeof item === "object" && !Array.isArray(item) && typeof item.id === "string" && item.id).map((item) => [item.id, item]));
  const client = mapOf(pipelinesOf(clientConfig)), disk = mapOf(pipelinesOf(diskConfig));
  const ids = /* @__PURE__ */ new Set([...client.keys(), ...disk.keys()]);
...
function parseAuthUsersYaml(text) {
...
function readAuthAdminUsers(file) {
  let text = "";
  try {
  ...
  for (const [name2, record] of parseAuthUsersYaml(text)) if (record.role === "admin") admins.add(name2);
  return admins;
}
function resolveRequestAuth(ctx, req) {
  let auth;
  try {
    auth = ctx ? ctx.auth : void 0;
  } catch {
    auth = void 0;
  }
  if (!auth) {
    try {
      auth = ctx && typeof ctx.get === "function" ? ctx.get("auth") : void 0;
    } catch {
      auth = void 0;
    }
  }
  const sessions = auth && auth.sessions;
  if (!sessions || typeof sessions.getByToken !== "function") return void 0;
  const token = requestAuthToken(req);
  let session;
  try {
    session = token ? sessions.getByToken(token) : void 0;
  } catch {
    session = void 0;
  }
...
function deepEqualIgnoring(a, b, ignoreKeys) {
  if (a === b) return true;
...
var BUILTIN_PIPELINE_ID = "pl-xds";
function isBuiltinPipelineEntry(entry) {
  return !!entry && typeof entry === "object" && !Array.isArray(entry) && (entry.builtIn === true || entry.id === BUILTIN_PIPELINE_ID);
}
function trustedPipelineViolations(storedConfig, mergedConfig) {
  const pipelinesOf = (value) => value && typeof value === "object" && !Array.isArray(value) && Array.isArray(value.pipelines) ? value.pipelines : [];
  const mapOf = (items) => new Map(items.filter((item) => item && typeof item === "object" && !Array.isArray(item) && typeof item.id === "string" && item.id).map((item) => [item.id, item]));
...
function trustedPipelineWriteDeny(ctx, req, storedConfig, mergedConfig) {
  ...
  return { message: parts.join("\uFF1B"), pipelineIds: [.../* @__PURE__ */ new Set([...violations.builtinEdit, ...violations.edit, ...violations.builtinCreate, ...violations.mark])] };
}
function mergePipelineHistoryForWrite(clientConfig, diskConfig, clientHistory, diskHistory) {
  ...
  const afterClear = (record) => !clearedAt || (Number(record && record.ts) || 0) > clearedAt;
  const acceptedClient = clientHistory.filter(afterClear);
  const keyOf = (record) => record && record.tag ? "tag:" + record.tag : record && record.ts ? "ts:" + record.ts : "no:" + (record && record.no) + ":" + (record && record.pipeline);
  const seen = new Set(acceptedClient.map(keyOf));
  const serverOnly = diskHistory.filter((record) => afterClear(record) && !seen.has(keyOf(record)));
  const history = acceptedClient.concat(serverOnly);
  history.sort((a, b) => (Number(b && b.ts) || 0) - (Number(a && a.ts) || 0));
  if (history.length > 500) history.length = 500;
  return { config, history };
}
async function handlePipelineHistoryRequest(req, res, deps) {
  try {
    if (req.method !== "GET") {
      res.writeHead(405);
      res.end();
...
async function readPipelineHistoryStore(file) {
  let raw;
  try {
...
function serverPromCollectScript(config) {
  ...
  return { name: name2, path: pathResolve(resolvePipelineScriptsDir(config, DEFAULT_PIPELINE_SCRIPTS_DIR), name2), params: [], values: {} };
}
...
function serverPipelineStageGroups(stages) {
...
function longestServerPromResult(results) {
  let longest = null;
...
function serverAbortReason(signal) {
...
function serverRequestTimeoutError() {
  const error = new Error("request timeout");
  error.code = "ETIMEDOUT";
  return error;
}
function serverTargetPolicyError(message) {
  const error = new Error(message);
...
function serverIpFamily(hostname) {
  const h = hostname.toLowerCase().replace(/^\[|\]$/g, "");
...
async function serverResolveLocalAddresses(hostname, signal, timeoutMs, resolveFn) {
  ...
  const records = await new Promise((resolve, reject) => {
    ...
    Promise.resolve().then(() => resolver(host, { all: true, verbatim: true })).then((value) => finish(null, value), (error) => finish(error));
  });
  const addresses = Array.isArray(records) ? records.map((record) => ({
    address: String(record && record.address || "").toLowerCase().replace(/^\[|\]$/g, ""),
    family: Number(record && record.family)
  })) : [];
  if (!addresses.length) {
    const error = new Error("\u76EE\u6807 DNS \u672A\u8FD4\u56DE\u5730\u5740");
    error.code = "ENOTFOUND";
    throw error;
  }
  const invalid = addresses.find((record) => record.family !== 4 && record.family !== 6 || serverIpFamily(record.address) !== record.family || !isLocalTarget(record.address));
  if (invalid) throw serverTargetPolicyError("\u76EE\u6807\u89E3\u6790\u5230\u4E86\u56DE\u73AF\u6216\u5185\u7F51\u4E4B\u5916\u7684\u5730\u5740\uFF1A" + invalid.address);
  return addresses;
}
function serverPinnedLookup(addresses) {
  let cursor = 0;
  return (_hostname, options, callback) => {
    const requestedFamily = typeof options === "number" ? Number(options) : Number(options && options.family);
    const candidates = requestedFamily === 4 || requestedFamily === 6 ? addresses.filter((record) => record.family === requestedFamily) : addresses;
    if (!candidates.length) {
      const error = new Error("validated target has no address for requested family");
      error.code = "ENOTFOUND";
      callback(error);
      return;
    }
...
async function serverDirectFetch(url, options = {}, timeoutMs = 2e4) {
  const startedAt = Date.now();
  const totalTimeoutMs = Math.max(1, Number(timeoutMs) || 2e4);
  ...
  if (signal && signal.aborted) throw serverAbortReason(signal);
  const addresses = options && options.requireLocalTarget === true ? await serverResolveLocalAddresses(target.hostname, signal, totalTimeoutMs, options.resolveFn) : null;
  if (signal && signal.aborted) throw serverAbortReason(signal);
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      try {
        request?.destroy(serverAbortReason(signal));
      } catch {
      }
    };
    const requestOptions = {
      ...
      agent: options && options.useProxy === true ? void 0 : new reqLib.Agent()
    };
    if (addresses) requestOptions.lookup = serverPinnedLookup(addresses);
    request = reqLib.request(url, requestOptions, (response) => {
    ...
    timer = setTimeout(() => {
      try {
        request.destroy(serverRequestTimeoutError());
      } catch {
      }
    }, Math.max(1, remainingMs));
    if (options && options.body !== void 0) request.write(options.body);
    request.end();
  });
}
function serverLocalFetch(url, options = {}, timeoutMs = 2e4) {
  return serverDirectFetch(url, { ...options || {}, requireLocalTarget: true }, timeoutMs);
}
function serverProxyTargetFetch(url, options, timeoutMs) {
  const target = new URL(url);
  if (!/^https?:$/.test(target.protocol)) throw new Error("unsupported protocol");
  ...
  return useProxy ? serverDirectFetch(url, options, timeoutMs) : serverLocalFetch(url, options, timeoutMs);
}
function registerWorktableProxyRoute(webServer) {
  webServer.register({
    kind: "exact",
    path: PROXY_PATH,
    handler: async (req, res) => {
      try {
        ...
        const upstream = await serverProxyTargetFetch(urlStr, { method, headers: fwdHeaders, body: reqBody, useProxy }, 2e4);
        const result = {
...
function abortableServerSleep(ms, signal) {
  return new Promise((resolve, reject) => {
    let timer = null;
    let listening = false;
    const cleanup = () => {
    ...
    const onAbort = () => {
      cleanup();
      reject(serverAbortReason(signal));
    };
...
function serverFetchErrorDetail(error) {
  ...
  let message = String(cause && cause.message || error && error.message || error || "unknown network error").replace(/[\r\n]+/g, " ").replace(/(https?:\/\/)[^@\s/]+@/gi, "$1***@").replace(/([?&](?:access[_-]?token|refresh[_-]?token|id[_-]?token|token|api[_-]?key|apikey|key|password|passwd|pass|secret|auth|authorization|credential|signature|sig)=)[^&\s]+/gi, "$1***").trim().slice(0, 500);
  if (!message) message = "unknown network error";
  return code && !message.toUpperCase().includes(code.toUpperCase()) ? code + ": " + message : message;
}
async function serverFetchResponse(fetchFn, url, options, deadline, label, maxBytes = PIPELINE_REMOTE_TEXT_LIMIT, signal) {
  if (signal && signal.aborted) throw serverAbortReason(signal);
  ...
  try {
    ...
    if (String(error && error.message || "").startsWith(label)) throw error;
    const wrapped = new Error(label + "\u8BF7\u6C42\u5931\u8D25\uFF1A" + serverFetchErrorDetail(error));
    wrapped.networkError = !(error && error.serverPolicyError);
    wrapped.cause = error;
    throw wrapped;
  } finally {
...
async function serverFetchJson(fetchFn, url, options, deadline, label, signal) {
...
async function serverFetchJsonWithRetry(fetchFn, url, options, deadline, label, sleep, signal) {
  for (let attempt = 1; ; attempt += 1) {
    try {
      return await serverFetchJson(fetchFn, url, options, deadline, label, signal);
    } catch (error) {
      const status = Number(error && error.httpStatus);
      const retryable = !!(error && error.networkError) || status === 408 || status === 429 || status >= 500 && status <= 599;
...
async function serverPostAction(fetchFn, url, headers, label, body = "") {
  const result = await serverFetchResponse(fetchFn, url, { method: "POST", headers, body }, Date.now() + 1e4, label, 64 * 1024);
  if (result.response.status < 200 || result.response.status >= 400) throw new Error(label + " HTTP " + result.response.status);
}
async function serverCancelJenkinsExecution(fetchFn, base, jobPath, buildNumber, queuePath, headers) {
  const actionHeaders = { ...headers };
  try {
    const crumb = await serverFetchJson(fetchFn, base + "/crumbIssuer/api/json", { method: "GET", headers }, Date.now() + 1e4, "Jenkins crumb");
  ...
  try {
    ...
    const queued = await serverFetchJson(fetchFn, base + queuePath.replace(/\/+$/, "") + "/api/json", { method: "GET", headers }, Date.now() + 1e4, "Jenkins \u961F\u5217\u72B6\u6001");
...
async function serverStopEvaltokensRun(fetchFn, base, runId, headers) {
...
function serverStartCleanupSignal(signal, graceMs = 1e4) {
  if (!signal) return { signal: void 0, dispose: () => {
  } };
...
async function executeServerHttpStage(stage, runCtx, config, varsPool, deps, signal) {
  const raw = serverStageUrl(stage);
  const deadline = serverStageDeadline(stage);
  let cancelJenkins = null;
  ...
  try {
    ...
    while (buildNumber === null) {
      ...
      if (buildNumber === null) await deps.sleep(1e3, signal);
    }
    let result = "";
    for (; ; ) {
    ...
    if (jenkinsActive && cancelJenkins) {
      try {
        ...
        message += "\nJenkins \u5916\u90E8\u4EFB\u52A1\u7EC8\u6B62\u5931\u8D25\uFF1A" + String(cancelError && cancelError.message ? cancelError.message : cancelError);
      }
    }
...
async function executeServerEvaltokensStage(stage, runCtx, config, varsPool, deps, signal) {
  const deadline = serverStageDeadline(stage);
  let stopRun = null;
  let runActive = false;
  try {
    ...
    try {
      ...
      }, deadline ? deadline + 1e4 : deadline, "EvalTokens \u542F\u52A8\u4EFB\u52A1", startGuard.signal);
    } finally {
      startGuard.dispose();
    }
    ...
    for (; ; ) {
      ...
      await deps.sleep(3e3, signal);
    }
  } catch (error) {
    let message = String(error && error.message ? error.message : error);
    const abortReason = signal && signal.aborted ? signal.reason : null;
    ...
    if (runActive && stopRun && !cascadeAbort) {
      try {
        ...
        message += "\nEvalTokens \u5916\u90E8\u4EFB\u52A1\u7EC8\u6B62\u5931\u8D25\uFF1A" + String(stopError && stopError.message ? stopError.message : stopError);
      }
    }
...
function buildServerTaskPromEnv(runCtx, config, varsPool, startMs, endMs, outputDir) {
  ...
  const resolved = (text) => text.indexOf("${") >= 0 ? "" : text;
...
function buildPipelineApiRun(store, pipelineId, body, runId) {
  ...
  if (requestedEnvironmentIds.length) {
    ...
    selectedEnvironments = [environments[0]];
  }
...
function pipelineLeaseOwner(plan) {
  const id = plan && (plan.runId || plan.id);
  return id ? String(id) : "plan-unknown";
}
function createPipelineNodeLeases(ttlMs = 90 * 1e3, nowFn = () => Date.now()) {
  const leases = /* @__PURE__ */ new Map();
  const sweep = () => {
    const t = nowFn();
    for (const [ip, l] of leases) if (t - l.seenAt > ttlMs) leases.delete(ip);
  };
  return {
    /** 原子申请：全部目标 IP 空闲（或本就由 owner 持有）才占用/续租；任一被他人占用则一个都不占。 */
    acquire(owner, ips, label = "", by = "") {
      sweep();
      const conflicts = [];
      for (const ip of ips) {
    ...
    release(owner, ips) {
      ...
      for (const [ip, l] of leases) {
        if (l.owner === owner && (!ips || !ips.length || ips.includes(ip))) {
          leases.delete(ip);
          n += 1;
        }
      }
    ...
    list() {
      sweep();
      const t = nowFn();
      return [...leases].map(([ip, l]) => ({ ip, owner: l.owner, label: l.label, by: l.by, since: l.since, seenAgo: Math.max(0, Math.round((t - l.seenAt) / 1e3)) }));
    }
  };
}
function createPipelineExecutionQueue(execute, limit = 2, queueLimit = 100, hooks) {
  ...
  let active = 0;
  let retryTimer = null;
  const retryWait = Math.max(50, Math.floor(Number(hooks && hooks.retryMs) || 5e3));
  const ipsOf = (plan) => {
    if (!hooks || !hooks.ipsOf) return [];
    try {
      return hooks.ipsOf(plan) || [];
    } catch {
      return [];
    }
  };
  const acquire = (plan, ips) => {
    if (!hooks || !hooks.acquire) return true;
    try {
      return hooks.acquire(plan, ips) !== false;
    } catch (err) {
      console.warn("[tokens-worktable] \u8282\u70B9\u79DF\u7EA6\u7533\u8BF7\u5F02\u5E38\uFF08\u6309\u53EF\u7528\u5904\u7406\uFF09:", err);
      return true;
    }
  };
  const release = (plan) => {
    ...
    try {
      ...
      console.warn("[tokens-worktable] \u8282\u70B9\u79DF\u7EA6\u91CA\u653E\u5F02\u5E38\uFF08\u79DF\u7EA6\u5230\u671F\u4F1A\u81EA\u52A8\u6E05\u7406\uFF09:", err);
    }
  };
  const runIdOf = (plan) => String(plan && (plan.runId || plan.id) || "");
  const stageList = (value) => Array.isArray(value) ? value.filter((stage) => stage && typeof stage === "object") : [];
  const idleNode = () => ({ status: "idle", progress: 0, dur: 0 });
  const liveLogLimit = 256 * 1024;
  const clipLiveLogTail = (value) => {
  ...
  const writeLog = (item, stageId, value, append) => {
  ...
  const updateStage = (item, stageId, state) => {
    const id = String(stageId || "");
    if (!id) return;
  ...
  const snapshotEntry = (item, timeKey) => ({
  ...
  const drain = () => {
    ...
    while (active < concurrency && pending.length) {
      ...
      for (let i = 0; i < pending.length; i++) {
        ...
        if (ips.length) {
          if (running.some((r) => r.ips.some((ip) => ips.includes(ip)))) continue;
          if (!acquire(pending[i].plan, ips)) continue;
        }
        picked = i;
        pickedIps = ips;
        break;
      ...
      const runtime = {
        ...
        setStages: (stages) => resetStages(item, stages),
        updateStage: (stageId, state) => updateStage(item, stageId, state),
        replaceLog: (stageId, text) => writeLog(item, stageId, text, false),
        appendLog: (stageId, text) => writeLog(item, stageId, text, true)
      };
      const settle = () => {
        active -= 1;
...
function apply(ctx) {
  ...
  const readVersionHistory = async () => {
    let raw = "";
    try {
      raw = await readFile(VERSION_HISTORY_FILE, "utf8");
    } catch {
      return [];
    }
    try {
      return sanitizeVersionHistory(JSON.parse(raw.charCodeAt(0) === 65279 ? raw.slice(1) : raw));
    } catch {
      return [];
    }
  };
  void (async () => {
    try {
      ...
      ctx.logger?.warn("[tokens-worktable] \u7248\u672C\u5B89\u88C5\u5386\u53F2\u5199\u5165\u5931\u8D25: " + String(err));
    }
  })();
  webServer.register({
    kind: "exact",
  ...
  const appendUsageEvent = (ev) => {
    ...
    usageWriteChain = p.then(() => void 0, () => void 0);
    return p;
  };
  webServer.register({
    kind: "exact",
    ...
    handler: async (req, res) => {
      if (req.method === "POST") {
        try {
          ...
          if (!ev) {
            json(res, 400, { error: "invalid usage event" });
            return;
          }
      ...
      try {
        ...
        if (req.method === "PUT") {
          ...
          const outcome = await withStoreLock(async () => {
            ...
            if (trustedDeny) return { conflicts: [], config: diskCfg, trusted: trustedDeny };
            ...
            return { conflicts: [], config: merged.config };
          });
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (!pipeline || !id) {
          json(res, 400, { error: "invalid pipeline" });
          return;
        }
        if (JSON.stringify(pipeline).length > 1024 * 1024) {
          json(res, 413, { error: "too large" });
          return;
        }
  ...
  webServer.register({
    ...
    handler: (req, res) => handlePipelineHistoryRequest(req, res, {
      statStore: async () => {
        try {
          return await fsStat(PIPELINE_STORE);
        } catch (error) {
  ...
  const QUEUE_NODE_STATUSES = /* @__PURE__ */ new Set(["idle", "running", "success", "failed", "skipped", "aborted"]);
  function cleanQueueStage(stage) {
    if (!stage || typeof stage !== "object" || typeof stage.id !== "string" || !stage.id) return null;
    const out = { id: stage.id.slice(0, 128), name: String(stage.name ?? "").slice(0, 200) };
    ...
    if (Array.isArray(stage.sub)) {
      const sub = stage.sub.slice(0, QUEUE_SUB_CAP).map((name2) => String(name2).slice(0, 128));
      if (sub.length) out.sub = sub;
    }
    return out;
  }
  function cleanQueueNode(node) {
    ...
    if (raw.sub && typeof raw.sub === "object" && !Array.isArray(raw.sub)) {
      ...
      for (const name2 of Object.keys(raw.sub).slice(0, QUEUE_SUB_CAP)) {
        const value = raw.sub[name2];
        if (QUEUE_NODE_STATUSES.has(value)) sub[String(name2).slice(0, 128)] = value;
      }
  ...
  function cleanQueueEntry(e, timeKey) {
    ...
    o.stages = (Array.isArray(e.stages) ? e.stages : []).slice(0, QUEUE_STAGE_CAP).map(cleanQueueStage).filter((stage) => !!stage);
      ...
      try {
        if (req.method === "PUT" || req.method === "POST") {
          ...
          if (req.method === "POST" && body.action === "cancel") {
            ...
            const result = pipelineExecutions && typeof pipelineExecutions.cancel === "function" ? pipelineExecutions.cancel(runId) : { ok: false, state: "missing" };
        ...
        if (req.method === "GET") {
          ...
          if (logRunId !== null || logStageId !== null) {
            if (!logRunId || !logStageId || logRunId.length > 128 || logStageId.length > 128) {
              json(res, 400, { error: "invalid runId or stageId" });
              return;
            }
            ...
            const log = pipelineExecutions && typeof pipelineExecutions.log === "function" ? pipelineExecutions.log(logRunId, logStageId) : null;
            if (!log) {
              json(res, 404, { error: "run or stage not found" });
              return;
            }
          ...
          for (const v of queuePresence.values()) {
            ...
            clients.push({ id: v.id, label: v.label, schemaVersion: v.schemaVersion, seenAgo: Math.max(0, Math.round((now - v.seenAt) / 1e3)), running: v.running, runs: v.runs, queue: v.queue });
          }
          const snapshot = pipelineExecutions && typeof pipelineExecutions.snapshot === "function" ? pipelineExecutions.snapshot() : { runs: [], queue: [] };
          const serverRuns = (Array.isArray(snapshot.runs) ? snapshot.runs : []).slice(0, 100).map((run) => cleanQueueEntry(run, "startedAt")).filter((run) => !!run);
          const serverQueue = (Array.isArray(snapshot.queue) ? snapshot.queue : []).slice(0, 100).map((run) => cleanQueueEntry(run, "queuedAt")).filter((run) => !!run);
          json(res, 200, {
            clients,
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        if (req.method === "GET") {
          json(res, 200, { leases: pipelineNodeLeases.list() });
          return;
        }
        if (req.method !== "POST") {
          res.writeHead(405);
        ...
        if (Array.isArray(body.runIds)) for (const id of body.runIds) {
          const s = String(id ?? "");
          if (s && owners.length < 64 && !owners.includes(s)) owners.push(s.slice(0, 128));
        }
        if (!owners.length) {
          json(res, 400, { error: "missing runId" });
          return;
        }
        const ips = (Array.isArray(body.ips) ? body.ips : []).map((ip) => String(ip ?? "").trim()).filter(Boolean).slice(0, 64);
        if (body.action === "release") {
          let released = 0;
          for (const owner of owners) released += pipelineNodeLeases.release(owner, ips.length ? ips : void 0);
        ...
        if (!ips.length) {
          json(res, 400, { error: "missing ips" });
          return;
        }
  ...
  const GPU_PROBE_MEM = `nvidia-smi --query-compute-apps=pid,used_memory --format=csv,noheader,nounits 2>/dev/null; echo ===; for p in $(nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null); do echo "$p:$(awk '/VmRSS/{print $2}' /proc/$p/status 2>/dev/null)"; done; echo ===; for p in $(nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null); do b=""; v2=$(grep "^0::" /proc/$p/cgroup 2>/dev/null | head -1 | cut -d: -f3-); if [ -n "$v2" ] && [ -r "/sys/fs/cgroup$v2/memory.current" ]; then b=$(cat "/sys/fs/cgroup$v2/memory.current" 2>/dev/null); fi; if [ -z "$b" ]; then v1=$(grep ":memory:" /proc/$p/cgroup 2>/dev/null | head -1 | cut -d: -f3-); if [ -n "$v1" ] && [ -r "/sys/fs/cgroup/memory$v1/memory.usage_in_bytes" ]; then b=$(cat "/sys/fs/cgroup/memory$v1/memory.usage_in_bytes" 2>/dev/null); fi; fi; echo "$p:$b"; done`;
  function parseHostPort(ip) {
    const m = /^([^:]+):(\d+)$/.exec(ip.trim());
    if (m) return { host: m[1], port: m[2] };
    return { host: ip.trim(), port: "" };
  }
  async function queryGpu(ip, user, pass, detail) {
    let out;
    ...
    if (detail) {
      ...
      for (const l of (secs[5] || "").trim().split("\n")) {
        ...
        if (m) rssOf[m[1]] = (Number(m[2]) / 1024).toFixed(1);
      }
      for (const l of (secs[6] || "").trim().split("\n")) {
        const m = /^(\d+):(\d+)$/.exec(l.trim());
        if (m) cgMemOf[m[1]] = (Number(m[2]) / 1048576).toFixed(1);
      }
    }
    const procsOut = procs.map((p) => {
      ...
      const row = { pid: p.pid, uuid: p.uuid, proc: p.proc, container, up };
      if (detail) {
        row.gpuMem = gpuMemOf[p.pid] || "";
        row.rss = rssOf[p.pid] || "";
        row.cgMem = cgMemOf[p.pid] || "";
      }
      return row;
    });
    ...
    const gpusOut = gpus.map((g) => {
      const row = { index: g.index, util: g.util, mem: g.mem };
  ...
  function applyOutVars(spec, pool, jsonCtx, fullText, sink) {
  ...
  async function runStageScript(sc, runCtx, scriptsDir, varsPool, timeoutSec, extraEnv, outputFile, signal, onLog) {
    const pool = varsPool || {};
    ...
    return new Promise((resolvePromise) => {
      ...
      const appendStdout = (text) => {
        ...
        emitLog(formatLogChunk(text, "stdout"));
      };
      const appendStderr = (text) => {
        appendServerTextTail(stderrTail, text);
        emitLog(formatLogChunk(text, "stderr"));
      };
      const killTree = () => {
        try {
  ...
  async function execPlan(pl, runtime) {
    const t0 = Date.now();
    ...
    const scriptsDir = resolvePipelineScriptsDir(cfg, DEFAULT_PIPELINE_SCRIPTS_DIR);
    ...
    const executeStage = async (s, index, baseVars, signal) => {
      const startedAt = Date.now();
      ...
      if (s.skip || s.gate) {
        ...
        let r;
        try {
          const directLog = folder ? taskLogPath(folder, tag, baseSeq + index + 1, s.name) : void 0;
          r = await runStageScript(s.script, runCtx, scriptsDir, localPool, s.timeout, void 0, directLog, signal, (text) => runtime?.appendLog?.(stageId, text));
        } catch (error) {
          r = { code: 1, stdout: "", stderr: String(error && error.message ? error.message : error), ...signal.aborted ? { aborted: true } : {} };
        }
        if (!r.aborted) {
          Object.assign(varsOut, r.vars || parseStageVars(r.stdout));
          Object.assign(localPool, varsOut);
          applyOutVars(s.script.outVars, localPool, r.json === void 0 ? parseStageJson(r.stdout) : r.json, r.fullTextOverflow ? void 0 : r.fullText === void 0 ? r.stdout : r.fullText, varsOut);
        }
        ...
        try {
          await abortableServerSleep(Math.min(Math.max(1, Number(s.dur) || 5), 60) * 1e3, signal);
    ...
    const collectStageProm = async (result) => {
      ...
      try {
        ...
        const pr = await runStageScript(collectScript, runCtx, scriptsDir, result.varsPool, 0, penv, promLog);
        if (!pr.logFile) try {
          const fsx = await import("node:fs/promises");
        ...
        addNote("[\u666E\u7F57\u91C7\u96C6] \u5931\u8D25\uFF08" + String(e && e.message ? e.message : e) + "\uFF0C\u4E0D\u5F71\u54CD\u4EFB\u52A1\u7ED3\u679C\uFF09");
        console.warn("[prom] \u4EFB\u52A1\u300C" + s.name + "\u300D\u666E\u7F57\u91C7\u96C6\u5F02\u5E38\uFF08\u4E0D\u5F71\u54CD\u6267\u884C\uFF09:", e);
      }
    };
    const recordStageResult = async (result, seq) => {
      const s = result.stage;
    ...
    for (const group of serverPipelineStageGroups(executionStages)) {
      ...
      let results;
      try {
        const jobs = group.stages.map((stage, offset) => executeStage(stage, group.start + offset, groupBase, controller.signal).then((result) => {
  ...
  pipelineExecutions = createPipelineExecutionQueue(execPlan, 2, 100, {
    ...
    ipsOf: (plan) => pipelineEnvIps(plan && plan.envs),
    acquire: (plan, ips) => pipelineNodeLeases.acquire(pipelineLeaseOwner(plan), ips, String(plan && plan.pipelineName || ""), String(plan && plan.by || "")).ok,
    release: (plan) => {
      pipelineNodeLeases.release(pipelineLeaseOwner(plan));
    }
  });
