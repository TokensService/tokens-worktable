...
// src/index.ts
var PLUGIN_VERSION = false ? "dev" : "1.0.6";
...
function apply(ctx) {
  ...
  function runStageScript(sc, runCtx, scriptsDir, varsPool, timeoutSec, extraEnv, signal) {
    const pool = varsPool || {};
    ...
    return new Promise((resolve) => {
      ...
      execFile(
        isPy ? "python3" : "bash",
        [sc.path, ...args],
        { cwd: scriptsDir || void 0, env: { ...process.env, ...env }, timeout: timeoutMs, maxBuffer: 256 * 1024 * 1024, windowsHide: true, signal },
        (err, stdout, stderr) => {
          const code = err ? typeof err.code === "number" ? err.code : 1 : 0;
          const aborted = !!(signal && signal.aborted);
          let errText = String(stderr || "");
          if (err && err.killed && !aborted) {
  ...
  async function execPlan(pl) {
    ...
    const executeStage = async (s, index, baseVars, signal) => {
      ...
      if (s.skip || s.gate) {
        ...
        let r;
        try {
          r = await runStageScript(s.script, runCtx, scriptsDir, localPool, s.timeout, void 0, signal);
        } catch (error) {
    ...
    const collectStageProm = async (result) => {
      ...
      try {
        ...
        const pr = await runStageScript(collectScript, runCtx, scriptsDir, result.varsPool, 0, penv);
        try {
          const fsx = await import("node:fs/promises");
        ...
        result.text += "\n[\u666E\u7F57\u91C7\u96C6] \u5931\u8D25\uFF08" + String(e && e.message ? e.message : e) + "\uFF0C\u4E0D\u5F71\u54CD\u4EFB\u52A1\u7ED3\u679C\uFF09";
