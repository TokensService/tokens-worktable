...
var PIPELINE_RUN_PRESETS = /* @__PURE__ */ new Set(["cleanup", "check", "profiling", "promCollect"]);
...
function serverPresetScript(key, config) {
  ...
  if (key === "promCollect") {
    const name2 = config.prom && typeof config.prom.collectScript === "string" ? config.prom.collectScript.trim() : "";
...
function materializeServerPipelineStages(stages, presets, config) {
  ...
  const missingBack = ["profiling", "promCollect"].filter((key) => selected.has(key) && !seen.has(key)).map((key) => materialize({}, key));
...
function buildServerPromCollectEnv(stage, runCtx, config, varsPool, runStartedAt) {
  const detail = stage && stage.prom && typeof stage.prom === "object" ? stage.prom : {};
  const parseFixed = (value) => value ? new Date(String(value)).getTime() : null;
  ...
  const model = substituteServerRunVars(detail.modelName === void 0 ? "${MODEL_PATH}" : detail.modelName, vars).trim();
  const namespace = substituteServerRunVars(detail.xdsNamespace === void 0 ? "${DEPLOY_STRATEGY}-${BY}" : detail.xdsNamespace, vars).trim();
  if (model) {
    env.ARCH_NAME = model;
...
function apply(ctx) {
  ...
  function runStageScript(sc, runCtx, scriptsDir, varsPool, timeoutSec, extraEnv) {
    const pool = varsPool || {};
    ...
    for (const s of executionStages) {
      ...
      if (s.skip || s.gate) {
        ...
        try {
          ...
          r = await runStageScript(s.script, runCtx, scriptsDir, varsPool, timeout, extraEnv);
        } catch (error) {
