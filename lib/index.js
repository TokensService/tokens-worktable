...
// src/index.ts
var PLUGIN_VERSION = false ? "dev" : "1.1.1";
...
function apply(ctx) {
  ...
  async function runStageScript(sc, runCtx, scriptsDir, varsPool, timeoutSec, extraEnv, outputFile, signal) {
    const pool = varsPool || {};
    ...
    const executeStage = async (s, index, baseVars, signal) => {
      ...
      if (s.skip || s.gate) {
        ...
        try {
          ...
          r = await runStageScript(s.script, runCtx, scriptsDir, localPool, s.timeout, void 0, directLog, signal);
        } catch (error) {
