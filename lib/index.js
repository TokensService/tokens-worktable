...
function buildPipelineApiRun(store, pipelineId, body, runId) {
  ...
  let selectedEnvironments = requestedEnvironmentIds.map((id) => environments.find((item) => item.id === id)).filter(Boolean);
  ...
  let repository = repositories.find((item) => item.id === repositoryId);
