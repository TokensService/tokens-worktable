...
export const css = xtermCss + '\n' + [
  ...
  /* 发布按钮（☁）：data-on=true 表示已发布（服务端同步、跨浏览器可见） */
  '.dsh-wt_managePublish[data-on=true]{color:var(--dsw-alias-state-accent-primary,#4f8ef7)}',
  ...
  // 项目卡片上的 💬 聊天按钮（打开/关闭该项目会话窗口）：绑定按钮左侧同款 22×22 按钮底，
  // 常态次级灰（--dsw-alias-label-secondary，与绑定按钮常态同色）；会话窗已关闭（data-closed=true）降透明度 + 去色，
  // 与分栏标题栏 💬 关态一致；气泡复用 .dsh-wt_bindTip（data-tip 事件委托）
  '.dsh-wt_chatBtn{position:absolute;right:50px;top:50%;transform:translateY(-50%);width:22px;height:22px;margin:0;display:flex;align-items:center;justify-content:center;border-radius:6px;color:var(--dsw-alias-label-secondary,#9aa4b2);font-size:12px;line-height:1;cursor:pointer;user-select:none}',
  '.dsh-wt_chatBtn:hover{background:var(--dsw-alias-fill-l2,rgba(255,255,255,.09));color:var(--dsw-alias-label-primary,#e6e8eb)}',
  '.dsh-wt_chatBtn[data-closed=true]{opacity:.45;filter:grayscale(1)}',
  '.dsh-wt_chatBtn[data-closed=true]:hover{opacity:.9}',
  ...
  // 项目设置弹窗内的自定义提示词编辑（复用 pageEditPrompt 主题变量样式，弹窗内更紧凑）
  '.dsh-wt_bindPrompt{margin-top:4px;min-height:60px}',
  '.dsh-wt_bindPrompt::placeholder{color:var(--dsw-alias-label-tertiary,#6b7280)}',
  ...
  // 应用内目录选择弹窗（native pickDirectory 不可用时的回退）
  '.dsh-wt_dirPickPop{gap:6px;padding:8px}',
  '.dsh-wt_dirPickCrumbs{display:flex;flex-wrap:wrap;align-items:center;gap:2px}',
  '.dsh-wt_dirPickCrumb{max-width:120px;padding:1px 4px;border:none;border-radius:4px;background:transparent;color:var(--dsw-alias-state-accent-primary,#4f8ef7);font:inherit;font-size:11px;line-height:16px;cursor:pointer;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
  '.dsh-wt_dirPickCrumb:hover{background:var(--dsw-alias-fill-l1,rgba(255,255,255,.05))}',
  '.dsh-wt_dirPickSep{color:var(--dsw-alias-label-tertiary,#6e7683);font-size:11px}',
  '.dsh-wt_dirPickInput{flex:none;width:100%}',
  '.dsh-wt_dirPickNew{display:flex;align-items:center;gap:6px}',
  '.dsh-wt_dirPickNew .dsh-wt_bindFolderChange{padding:4px 10px;line-height:18px}',
  '.dsh-wt_dirPickNew .dsh-wt_bindFolderChange:disabled{opacity:.5;cursor:default}',
  '.dsh-wt_dirPickList{margin-top:0;max-height:240px}',
  '.dsh-wt_dirPickMsg{padding:10px 8px;font-size:11.5px;line-height:17px;color:var(--dsw-alias-label-tertiary,#6e7683);text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
  '.dsh-wt_dirPickErr{color:var(--dsw-alias-state-danger,#f85149)}',
  '.dsh-wt_dirPickOk{align-self:stretch}',
  // 自定义窗完成页：自动绑定提示
  ...
  // 会话窗开关（关闭会话窗口 / 重新打开；关闭态置灰提示）；常态次级灰（与绑定按钮常态同色）
  '.dsh-wt_splitChatToggle{flex:none;display:flex;align-items:center;justify-content:center;width:20px;height:20px;padding:0;border:none;border-radius:4px;background:transparent;color:var(--dsw-alias-label-secondary,#9aa4b2);font-size:12px;cursor:pointer}',
  '.dsh-wt_splitChatToggle:hover{color:var(--dsw-alias-label-primary,#e6e8eb);background:var(--dsw-alias-fill-l1,rgba(255,255,255,.06))}',
  '.dsh-wt_splitChatToggleOff{opacity:.45;filter:grayscale(1)}',
  '.dsh-wt_splitChatToggleOff:hover{opacity:.9}',
  // 💬 对话气泡图标（CSS 绘制：圆角描边泡体 + 左下实心三角尾，颜色随 currentColor——
  // emoji 💬 无法染色，CSS 绘制才能随主题配色并随关态 grayscale 置灰；分栏标题栏与项目行 💬 按钮共用）
  '.dsh-wt_chatGlyph{position:relative;flex:none;width:12px;height:9px;box-sizing:border-box;border:1.5px solid currentColor;border-radius:3px}',
  '.dsh-wt_chatGlyph::after{content:\'\';position:absolute;left:0;top:100%;margin-top:-1px;width:0;height:0;border:3px solid transparent;border-top-color:currentColor;border-left-color:currentColor}',
  // + 右侧弹出面板
  ...
  '.dsh-wt_histItem{border:1px solid var(--dsw-alias-border-l1,#262b36);border-radius:8px;padding:6px 8px}',
  '.dsh-wt_histItem[data-current=true]{border-color:var(--dsw-alias-state-accent-primary,#4f8ef7)}',
  '.dsh-wt_histItemHead{display:flex;align-items:center;gap:6px;cursor:pointer;user-select:none}',
  '.dsh-wt_histCaret{width:10px;flex:none;font-size:9px;color:var(--dsw-alias-label-tertiary,#7d8aa5)}',
  ...
  '.dsh-wt_histNew{flex:none;padding:1px 6px;border-radius:999px;border:1px solid rgba(245,185,66,.45);background:rgba(245,185,66,.18);color:#f7d488;font-size:10px}',
  '.dsh-wt_histDate{margin-left:auto;flex:none;font-size:10.5px;color:var(--dsw-alias-label-tertiary,#7d8aa5)}',
  '.dsh-wt_histNotes{margin-top:6px;padding-top:6px;border-top:1px dashed var(--dsw-alias-border-l1,#262b36);max-height:220px;overflow:auto;font-size:11px;line-height:1.6;color:var(--dsw-alias-label-tertiary,#7d8aa5);white-space:pre-wrap;word-break:break-word}',
  '.dsh-wt_histLink{display:inline-block;margin-top:6px;font-size:10.5px;color:var(--dsw-alias-state-accent-primary,#4f8ef7);text-decoration:none}',
].join('\n')
