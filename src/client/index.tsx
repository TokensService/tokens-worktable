...
type ProjectsState = {
  ...
  /** 项目 → 会话分组（宿主工作区 id）：从该项目新建会话（如「页面修改」）时落进这个分组；空/缺 = 未分组。 */
  workspaces: Record<string, string>
  /** 项目 → 自定义「页面修改」提示词模板：该项目点 ✏️ 时优先用这份（{page}/{name} 占位符同全局模板）；空/缺 = 用设置面板的全局模板。 */
...
/** 发布按钮的云图标：SVG fill=currentColor，发布后随 data-on 整体变蓝
 * （emoji ☁ 是彩色字形、忽略 CSS color，做不到变蓝，故不用）。 */
const ICON_CLOUD = (
...
/** 由绝对路径推出面包屑链（根 → 当前目录，逐级可跳转），与宿主 browse 的 ancestryCrumbs 同形。
 *  仅用于「宿主 uiWorkspace.listDirectory 不可用」回退到插件自身 /api/worktable/fs 时的结果适配；
 *  根级 crumb 取完整路径为名（同宿主约定），非根级取 basename。 */
function crumbsOf(abs: string): { name: string; path: string; hidden: false }[] {
  if (!abs) return []
  const chain: { name: string; path: string; hidden: false }[] = []
  let cur = abs
  let guard = 0
  while (cur && guard++ < 64) {
    const parent = parentPathOf(cur)
    chain.push({ name: cur === parent ? cur : basenameOf(cur), path: cur, hidden: false })
    if (cur === parent) break
    cur = parent
  }
...
/** 全局默认会话分组（设置面板「工作区」设置项；读 localStorage 视图态切片，供模块级新建会话路径使用）。
 *  引用的工作区已被删除时按未设置处理（避免带着失效 id 建会话）。 */
function defaultWorkspaceId(): string | null {
  try {
    const raw = localStorage.getItem(PERSIST_KEY)
    if (!raw) return null
    const id = JSON.parse(raw)?.workspace
...
const KNOWLEDGE_PACK = [
  ...
  '- 改完插件在插件根目录执行 npm run build；重启 dsh web 或浏览器 F5 生效。',
...
export async function createCustomSession(projectId: string, projectName: string, requirement: string, group?: NewSessionGroup, windowLabel = '', folder: string | null = null): Promise<string> {
  ...
  if (g.kind === 'existing') {
    ...
    try { await applyCtx?.get?.('uiWorkspace')?.createDirectory?.(parent, name) } catch { /* 宿主目录能力不可用（未组合 / native）→ 落入插件 mkdir 兜底 */ }
    try {
...
/** AI 日志分析（pipeline.html postMessage 触发）：新开会话，把日志文件路径交给聊天 Agent 分析 */
async function analyzeLogInSession(logPath: string): Promise<void> {
  const b = sessionBridge
  if (!b || typeof b.sessions?.create !== 'function') throw new Error('sessions unavailable')
  const text = [
    '【流水线日志分析】',
    '请阅读并分析流水线运行日志文件：' + logPath,
    '（内含多次运行记录：每次运行的状态/耗时/执行人，以及各阶段日志；脚本阶段含真实 stdout/stderr 与 exit code。）',
    '请给出：1) 整体运行情况概述（成功率、趋势）；2) 失败/异常运行的根因分析；3) 可执行的改进建议。',
  ].join('\n')
  // 未指定目录的分析会话：落进设置面板的默认会话分组（未设置 = 未分组）
  const wsId = defaultWorkspaceId()
  const sessionId = await b.sessions.create(wsId ? { workspaceId: wsId } : {})
...
/** 内容页（pipeline.html「页面修改」按钮等）→ 右侧聊天窗桥（经 window.__dshNewChatSession 暴露给 iframe 调用）：
 *  新建会话并把提示词填入输入框（不提交，用户确认后手动发送）；标记为插件发起的切换，
 *  项目分栏保持打开，新会话出现在右侧聊天窗而非离开项目页。
 *  workspaceId 为项目设置里选定的会话分组；cwd 为新会话工作目录
 *  （如流水线运行历史分析按钮传入选中行的日志归档目录）。
 *  分组/目录优先级：项目分组（workspaceId）> 指定目录（cwd）> 设置面板默认分组 > 未分组。 */
async function newChatInProject(text: string, workspaceId: string | null = null, cwd: string | null = null): Promise<void> {
  const b = sessionBridge
  if (!b || typeof b.sessions?.create !== 'function') throw new Error('sessions unavailable')
...
/** 内容页「一键 AI 生成」类按钮（如 code-review-prs.html 发行说明「✦ AI 生成」）→ 右侧聊天窗桥
 *  （经 window.__dshSendChatInProject 暴露给 iframe 调用）：与 newChatInProject 相同地新建会话、
 *  标记插件发起的切换使项目分栏保持打开、会话出现在右侧聊天窗；区别在于提示词直接自动发送
 *  （promptIntoSession），不经过输入框草稿——「一键生成」场景无需用户再点一次发送。
 *  分组/目录优先级同 newChatInProject。 */
async function sendChatInProject(text: string, workspaceId: string | null = null, cwd: string | null = null): Promise<void> {
  const b = sessionBridge
  if (!b || typeof b.sessions?.create !== 'function') throw new Error('sessions unavailable')
  const wsId = workspaceId ?? (cwd ? null : defaultWorkspaceId())
  const sessionId = await b.sessions.create(wsId ? { workspaceId: wsId } : (cwd ? { cwd } : {}))
 ...
 *  切换落定后重开 better-sidebar 文件夹窗口：会话切换会刷新侧边栏，项目页内先开的会被刷掉，
 *  故此处立即 + 延时各开一次（openTab 按 dedupeKey 复用同一标签，幂等）。
 *  text 非空时才填输入框草稿（不提交）；folder 为空（无 better-sidebar 等）则只建会话。 */
async function newChatSessionWithFolder(text: string, cwd: string | null, folder: string | null): Promise<void> {
  const b = sessionBridge
  if (!b || typeof b.sessions?.create !== 'function') throw new Error('sessions unavailable')
  ...
  if (folder) {
    ...
    setTimeout(() => openFolderInSidebar(folder), 400)
    setTimeout(() => openFolderInSidebar(folder), 1200)
  }
}

// ── 页面修改（设置弹窗：项目管理行「✏️」按钮 + 提示词模板配置；替代原 pipeline.html 页内按钮）──
/** 默认「页面修改」提示词模板：{page} 替换为项目页面文件路径，{name} 替换为项目显示名 */
const DEFAULT_PAGE_EDIT_PROMPT = '页面文件：{page}，这是一个deepseek harness worktable插件的页面\n请先问我想做什么修改；确认需求后直接编辑该文件，保持现有代码风格与 dshell 样式体系（单文件内联实现，主题变量适配浅色/深色）。修改完成后提醒我刷新页面查看效果。\n修改代码时，创建一个git worktree修改，防止多个并行的修改冲突。改完后，创建提交PR链接'
const PAGE_EDIT_PROMPT_KEY = 'dsh.worktable.pageEditPrompt.v1'
/** 读取「页面修改」提示词模板（localStorage；空值/未设 = 默认模板） */
function loadPageEditPrompt(): string {
  try { const p = localStorage.getItem(PAGE_EDIT_PROMPT_KEY); return (p == null || !String(p).trim()) ? DEFAULT_PAGE_EDIT_PROMPT : p } catch { return DEFAULT_PAGE_EDIT_PROMPT }
}
/** 保存「页面修改」提示词模板（存空串 = 恢复默认） */
function savePageEditPrompt(text: string): void {
  try { localStorage.setItem(PAGE_EDIT_PROMPT_KEY, text) } catch {}
}
/** 从布局各窗口的 iframe 标签推项目页面文件路径（/api/worktable/site/<编码目录>/<文件名> → 绝对路径）；非本机托管页返回 null */
function layoutPagePath(layout: any): string | null {
  const SITE = '/api/worktable/site/'
  const panels: any[] = [...(layout?.left ? [layout.left] : []), ...(layout?.top ? [layout.top] : []), ...(Array.isArray(layout?.main) ? layout.main : [])]
  for (const p of panels) {
    const tabs: any[] = Array.isArray(p?.tabs) ? p.tabs : (p?.content ? [{ content: p.content }] : [])
    for (const tb of tabs) {
      const u = tb?.content?.url
      if (typeof u === 'string' && u.startsWith(SITE)) {
        try { return u.slice(SITE.length).split('/').map((s) => decodeURIComponent(s)).join('/') } catch { return null }
      }
    }
  }
  return null
...
function worktableTitleOf(customTitle: string, authUsername: string, fallback: string): string {
  return customTitle || authUsername || fallback
}
/* ---------- 侧栏标题取 dsh 登录用户结束 ---------- */
...
function WorktableSection(props: any) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const r = await fetch('/api/worktable/projects', { cache: 'no-store' })
        const d = await r.json()
        if (!alive || !d || !Array.isArray(d.layouts)) return
        const remote = (d.layouts as any[])
          .filter((l) => l && typeof l.id === 'string' && typeof l.title === 'string' && Array.isArray(l.main))
          .map((l) => ({ ...l, sync: true as const }))
        const remoteFolders: Record<string, string> = {}
        if (d.folders && typeof d.folders === 'object') {
          for (const [k, v] of Object.entries(d.folders)) if (typeof v === 'string') remoteFolders[k] = v
        }
        const remoteWorkspaces: Record<string, string> = {}
        if (d.workspaces && typeof d.workspaces === 'object') {
          for (const [k, v] of Object.entries(d.workspaces)) if (typeof v === 'string') remoteWorkspaces[k] = v
        }
  ...
  /** 会话分组（工作区）列表弹层：点绑定弹窗「工作区」行弹出（与对话列表弹层互斥） */
  const [bindWsOpen, setBindWsOpen] = useState(false)
  /** 应用内目录选择弹窗：native pickDirectory 不可用（browse 组合 / 远程无桌面）时的回退；
   *  经宿主 browse 能力 listDirectory 逐层浏览，manual 为可编辑的当前路径（确认即取它） */
  ...
  // 当前打开项目的会话窗关/开状态（项目行 💬 按钮的按钮态；未打开项目读持久化存档）
  const [activeChatClosed, setActiveChatClosed] = useState<boolean>(() =>
    splitStore.active ? splitStore.chatClosed === true : false,
  )
  ...
  useEffect(() => {
    const env = {
      ...
      custom: {
        ...
        createWorkspaceDir: async (parent: string, name: string) => {
          try {
            const uiWs = applyCtx?.get?.('uiWorkspace') as any
            if (uiWs && typeof uiWs.createDirectory === 'function') {
              const p = await uiWs.createDirectory(parent, name)
              if (typeof p === 'string' && p) return p
            }
          } catch { /* 宿主目录能力不可用 → 落入插件 mkdir 兜底 */ }
          const full = joinPath(parent, name)
          const r = await fetch('/api/worktable/mkdir', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ path: full }) })
          const d = await r.json()
          if (!r.ok || !d?.ok) throw new Error(d?.error ?? 'mkdir failed')
          return typeof d.path === 'string' && d.path ? d.path : full
        },
  ...
  const persistProjects = (patch: Partial<ProjectsState> | ((prev: ProjectsState) => ProjectsState)) => {
    setProjects((prev) => {
      ...
      const local = {
        ...next,
        layouts: next.layouts.filter((l) => !l.sync),
        folders: Object.fromEntries(Object.entries(next.folders).filter(([id]) => !syncedIds.has(id))),
        workspaces: Object.fromEntries(Object.entries(next.workspaces).filter(([id]) => !syncedIds.has(id))),
      ...
      if (prevSync !== nextSync) {
        void fetch('/api/worktable/projects', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: nextSync }).catch(() => {})
      }
      return next
    })
  }
  ...
  useEffect(() => splitStore.subscribe(() => {
    ...
    setActiveChatClosed(splitStore.active ? splitStore.chatClosed === true : false)
...
function buildCustomLayoutPrompt(req: string): string {
  return [
    ...
    '8. 改完在插件根目录执行 npm run build 重新打包，浏览器 F5 即生效。',
  ...
  /** 项目行 💬 按钮的按钮态：当前打开项目读引擎实时状态，未打开项目读持久化存档（无 = 打开） */
  const rowChatClosed = (id: string) => (activeSplitId === id ? activeChatClosed : peekChatClosed(id))
  /** 项目行 💬 聊天按钮：打开/关闭该项目的会话窗口；项目未打开时先打开再切换（开/关状态随布局条目持久化） */
  const toggleRowChat = (id: string, open: () => void) => {
    if (!(splitStore.active && splitStore.spec?.id === id)) open()
    if (splitStore.active && splitStore.spec?.id === id) splitStore.setChatClosed(!splitStore.chatClosed)
  }
   ...
   *  仅保留子目录并按路径推面包屑），保证远程/未组合 browse 能力时目录选择仍可用；
   *  manualOverride 用于「列表留在当前层、确认目标指向另一路径」的场景（新建文件夹后选中新目录） */
  const dirPickLoad = async (path?: string, manualOverride?: string) => {
    const uiWs = applyCtx?.get?.('uiWorkspace') as any
    setDirPick((s) => (s ? { ...s, loading: true, error: null } : s))
    try {
      let d: any
      if (uiWs && typeof uiWs.listDirectory === 'function') {
        d = await uiWs.listDirectory(path)
      } else {
        const r = await fetch('/api/worktable/fs', {
          method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ path: path ?? '' }),
        })
        const j = await r.json()
        if (!r.ok || !Array.isArray(j?.entries)) throw new Error(j?.error ?? 'listDirectory failed')
        const base = typeof j.path === 'string' && j.path ? j.path : (path ?? '')
        const dirEntries = (j.entries as any[])
          .filter((e) => e?.isDir)
          .map((e) => ({ name: e.name, path: e.path, hidden: !!e.hidden }))
        d = { path: base, crumbs: crumbsOf(base), entries: dirEntries }
      }
      setDirPick((s) => (s
        ? {
            ...s,
            loading: false,
            path: typeof d?.path === 'string' ? d.path : s.path,
            crumbs: Array.isArray(d?.crumbs) ? d.crumbs : [],
            entries: Array.isArray(d?.entries) ? d.entries : [],
            manual: typeof manualOverride === 'string' ? manualOverride : (typeof d?.path === 'string' ? d.path : s.manual),
            newName: '',
          }
        : s))
    } catch (err) {
      setDirPick((s) => (s ? { ...s, loading: false, error: String(err) } : s))
    }
  }

  /** 目录选择弹窗内新建文件夹：优先经宿主 uiWorkspace.createDirectory 在当前目录下建单层目录；
   *  不可用时回退到插件自身 POST /api/worktable/mkdir（父目录必须已存在，避免误建深层目录）。
   *  成功后刷新当前目录列表（新文件夹立即可见）并把确认目标设为新文件夹——对齐宿主
   *  DirectoryBrowser「创建后重列当前层 + 选中新目录」的交互；直接进入新目录会让列表变空，
   *  看起来像「没有生效」。失败（重名 / 无权限等）在列表区显示原因 */
  const dirPickCreate = async () => {
    const cur = dirPick
    if (!cur) return
    const name = cur.newName.trim()
    if (!name) return
    if (/[/\\]/.test(name)) { setDirPick((s) => (s ? { ...s, error: t('dirpick.newInvalid') } : s)); return }
    if (!cur.path) { setDirPick((s) => (s ? { ...s, error: 'no current directory' } : s)); return }
    const uiWs = applyCtx?.get?.('uiWorkspace') as any
    setDirPick((s) => (s ? { ...s, loading: true, error: null } : s))
    try {
      let createdPath = ''
      if (uiWs && typeof uiWs.createDirectory === 'function') {
        createdPath = await uiWs.createDirectory(cur.path, name)
      } else {
        const full = joinPath(cur.path, name)
        const r = await fetch('/api/worktable/mkdir', {
          method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ path: full }),
        })
        const j = await r.json()
        if (!r.ok || !j?.ok) throw new Error(j?.error ?? 'mkdir failed')
        createdPath = typeof j.path === 'string' && j.path ? j.path : full
      }
      const createdPathStr = typeof createdPath === 'string' && createdPath ? createdPath : joinPath(cur.path, name)
      await dirPickLoad(cur.path, createdPathStr)
    } catch (err) {
      setDirPick((s) => (s ? { ...s, loading: false, error: String(err) } : s))
    }
  }

  /** 弹出系统文件夹选择窗（宿主 uiWorkspace.pickDirectory）；选中后回调。
   *  native 能力不可用（宿主未组合 directory-picker / 远程无桌面）时回退应用内目录浏览弹窗；
   *  用户在原生框里点取消（返回 null）不回退。 */
  const pickFolder = async (apply: (p: string) => void, startPath?: string) => {
    const uiWs = applyCtx?.get?.('uiWorkspace') as any
    try {
      if (uiWs && typeof uiWs.pickDirectory === 'function') {
        const p = await uiWs.pickDirectory()
        if (p && typeof p === 'string') apply(p)
        return
      }
    } catch { /* native 不可用 → 落入应用内浏览 */ }
    const start = typeof startPath === 'string' ? startPath.trim() : ''
    setDirPick({ apply, path: start, crumbs: [], entries: [], manual: start, newName: '', loading: true, error: null })
    dirPickLoad(start || undefined)
  }

  /** 绑定弹窗「更改」：弹窗选择文件夹，直接作为项目文件夹（起始定位到当前文件夹） */
  const changeBindFolder = () => {
    if (!bindPick) return
    pickFolder((p) => {
      persistProjects((prev) => ({ ...prev, folders: { ...prev.folders, [bindPick.id]: p } }))
    }, projectsRef.current.projects.folders[bindPick.id] ?? undefined)
  }

  /** 设置 / 清除项目的会话分组（工作区）：从该项目新建会话时落进选中的分组 */
  const setProjectWorkspace = (id: string, workspaceId: string | null) => {
    persistProjects((prev) => {
  ...
  const setProjectPrompt = (id: string, text: string | null) => {
    persistProjects((prev) => {
  ...
  const renameProject = (id: string, name: string) => {
    persistProjects((prev) => {
      // 同步布局改名直接写 title：nameOverrides 是本地偏好，不会跨浏览器
      const layout = prev.layouts.find((l) => l.id === id)
      if (layout?.sync) {
        const title = name.trim()
        if (!title || title === layout.title) return prev
        return { ...prev, layouts: prev.layouts.map((l) => (l.id === id ? { ...l, title } : l)) }
      }
  ...
  const saveLayout = async () => {
    ...
    const layout = buildLayout(wsPreset, name) // 新建一律本地；完善后在管理列表里「发布」才同步到服务端
  ...
  const togglePublish = (id: string) => {
    persistProjects((prev) => ({
      ...prev,
      layouts: prev.layouts.map((l) => (l.id === id ? { ...l, sync: l.sync ? undefined : true } : l)),
    }))
  }
  ...
  const removeProject = (id: string) => {
    ...
    persistProjects((prev) => {
      ...
      if (next.prompts[id]) { next.prompts = { ...next.prompts }; delete next.prompts[id] }
  ...
  // 本地编译安装（link:）时默认名追加「（开发中）」后缀（「工作台」本字不改；自定义名与登录用户名不受影响，改名框也不带后缀）
    ...
    <div ref={rootRef} className={'dsh-wt_section' + (isFloat ? ' dsh-wt_float' : '')} style={isFloat ? floatStyle : dockedStyle}>
        ...
        <div className="dsh-wt_menu dsh-wt_add dsh-wt_pop" style={{ position: 'fixed', left: popLeft, top: popTop, width: 360, zIndex: 80 }}>
          ...
          <div className="dsh-wt_addForm">
            ...
            <div className="dsh-wt_addFolderRow">
              ...
              <button type="button" className="dsh-wt_bindFolderChange" onClick={() => { pickFolder((p) => { setWsFolderParent(p); setWsFolderError(false) }, wsFolderParent || undefined); setWsError(false) }}>{t('add.folderPick')}</button>
            </div>
      ...
      {viewOptionsOpen && (
        <div ref={settingsRef} className="dsh-wt_manage dsh-wt_pop dsh-wt_settings" style={{ position: 'fixed', left: popLeft, top: settingsTop ?? popTop, width: 400, zIndex: 80 }}>
          ...
          <div className="dsh-wt_pageEditHint">{t('workspace.desc')}</div>
          {(() => {
            const wsList = listWorkspaces()
            const cur = view.workspace ?? ''
            // 所选工作区已被删除：保留一个占位项显示原 id，便于识别并改选/清除
            const missing = !!cur && !wsList.some((w) => w.id === cur)
            return (
              <select
                className="dsh-wt_consoleSelect"
                value={cur}
                onChange={(e) => persistView({ workspace: e.target.value || null })}
              >
                <option value="">{t('workspace.none')}</option>
                {missing && <option value={cur}>{cur} {t('workspace.missing')}</option>}
                {wsList.map((w) => <option key={w.id} value={w.id} title={w.path}>{w.title || w.path || w.id}</option>)}
              </select>
            )
          })()}
          <div className="dsh-wt_menuSep" />
          ...
          {effectiveOrder.map((id) => {
            ...
            const pagePath = layout ? layoutPagePath(layout) : null
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  onClick={(e) => { e.stopPropagation(); openBindPick(id, e.currentTarget as HTMLElement) }}
                >⚙</button>
                ...
                {pagePath && (
                  <button
                    type="button"
                    className="dsh-wt_manageBtn"
                    title={t('pageEdit.hint')}
                    onClick={() => { setViewOptionsOpen(false); void startPageEdit(pagePath, display, projects.workspaces[id] ?? null, projects.prompts[id]) }}
                  >✏️</button>
                )}
                {layout && (
                  <button
                    type="button"
                    className="dsh-wt_manageBtn dsh-wt_managePublish"
                    data-on={layout.sync ? 'true' : 'false'}
                    title={layout.sync ? t('manage.unpublish') : t('manage.publish')}
          ...
          <div className="dsh-wt_menuSep" />
          <div className="dsh-wt_manageHead">
            <span className="dsh-wt_manageTitle">{t('pageEdit.title')}</span>
          </div>
          <div className="dsh-wt_pageEditHint">{t('pageEdit.desc')}</div>
          <textarea
            className="dsh-wt_pageEditPrompt"
            rows={5}
            value={pageEditPrompt}
            onChange={(e: any) => { setPageEditPrompt(e.target.value); savePageEditPrompt(e.target.value) }}
          />
          <div className="dsh-wt_pageEditOps">
            <button type="button" className="dsh-wt_updateBtn" onClick={() => { setPageEditPrompt(DEFAULT_PAGE_EDIT_PROMPT); savePageEditPrompt('') }}>{t('pageEdit.reset')}</button>
          </div>
          <div className="dsh-wt_menuSep" />
          <div className="dsh-wt_manageHead">
            <span className="dsh-wt_manageTitle">{t('dev.title')}</span>
          </div>
          <div className="dsh-wt_pageEditHint">{t('dev.desc')}</div>
          <textarea
            className="dsh-wt_pageEditPrompt"
            rows={5}
            value={devPrompt}
            onChange={(e: any) => { setDevPrompt(e.target.value); saveDevPrompt(e.target.value) }}
          />
          <div className="dsh-wt_pageEditOps">
            <button type="button" className="dsh-wt_updateBtn" onClick={() => { setDevPrompt(DEFAULT_DEV_PROMPT); saveDevPrompt('') }}>{t('pageEdit.reset')}</button>
            <button type="button" className="dsh-wt_updateBtn" title={t('dev.hint')} onClick={() => { setViewOptionsOpen(false); void startPluginDev() }}>🛠 {t('dev.start')}</button>
          </div>
      ...
      {bindPick && (
        <div className="dsh-wt_menu dsh-wt_pop dsh-wt_bindPop" style={{ position: 'fixed', left: bindPick.x, top: bindPick.y, width: 280, zIndex: 84 }}>
          ...
          {/* 工作区（会话分组）框（与项目文件夹同格式）：第一行 🗂+标题（最右清除）；第二行选中的分组名（点击弹列表）；框内下方：说明 */}
          <div className="dsh-wt_bindFolderBox">
            <div className="dsh-wt_bindFolderRow">
              <span className="dsh-wt_bindFolderLabel">🗂 {t('bind.workspace')}</span>
              {projects.workspaces[bindPick.id] && (
                <button type="button" className="dsh-wt_bindUnbind" onClick={() => setProjectWorkspace(bindPick.id, null)}>{t('bind.workspaceClear')} ✕</button>
              )}
            </div>
            {(() => {
              const wid = projects.workspaces[bindPick.id]
              const cur = wid ? listWorkspaces().find((w) => w.id === wid) : undefined
              if (!wid) {
                return (
                  <button type="button" className="dsh-wt_bindConvRow dsh-wt_bindConvRowNone" title={t('bind.workspaceHint')} onClick={() => { setBindWsOpen((v) => !v); setBindListOpen(false) }}>
                    <span className="dsh-wt_bindNoneText">{t('bind.workspaceNone')} · {t('bind.clickPick')}</span>
                  </button>
                )
              }
              return (
                <button type="button" className="dsh-wt_bindConvRow" title={cur ? cur.path : wid} onClick={() => { setBindWsOpen((v) => !v); setBindListOpen(false) }}>
                  <span className="dsh-wt_bindConvName" title={cur ? cur.title : wid}>{cur ? cur.title : wid}</span>
                  <span className="dsh-wt_bindConvChevron" aria-hidden>▾</span>
                </button>
              )
            })()}
            <p className="dsh-wt_bindHint">{t('bind.workspaceHint')}</p>
          </div>
          ...
          <div className="dsh-wt_bindFolderBox">
            ...
            {(() => {
              ...
              if (!sid) {
                return (
                  <button type="button" className="dsh-wt_bindConvRow dsh-wt_bindConvRowNone" title={t('bind.tipUnbound')} onClick={() => { setBindListOpen((v) => !v); setBindWsOpen(false) }}>
              ...
              return (
                <button type="button" className="dsh-wt_bindConvRow" title={t('bind.tipBound', { name: info.title })} onClick={() => { setBindListOpen((v) => !v); setBindWsOpen(false) }}>
          ...
          {/* 自定义提示词框（与项目文件夹同格式；仅本项目有页面文件、✏️ 可用时显示）：
              第一行 📝+标题（最右清除）；第二行 textarea 编辑（留空 = 用全局模板）；框内下方：说明 */}
          {(() => {
            const bindLayout = projects.layouts.find((l) => l.id === bindPick.id)
            if (!bindLayout || !layoutPagePath(bindLayout)) return null
            return (
              <div className="dsh-wt_bindFolderBox">
                <div className="dsh-wt_bindFolderRow">
                  <span className="dsh-wt_bindFolderLabel">📝 {t('bind.prompt')}</span>
                  {projects.prompts[bindPick.id] && (
                    <button type="button" className="dsh-wt_bindUnbind" onClick={() => setProjectPrompt(bindPick.id, null)}>{t('bind.promptClear')} ✕</button>
                  )}
                </div>
                <textarea
                  className="dsh-wt_pageEditPrompt dsh-wt_bindPrompt"
                  rows={4}
                  value={projects.prompts[bindPick.id] ?? ''}
                  placeholder={t('bind.promptPh')}
                  onChange={(e: any) => setProjectPrompt(bindPick.id, e.target.value)}
                />
                <p className="dsh-wt_bindHint">{t('bind.promptHint')}</p>
              </div>
            )
          })()}
        </div>
      ...
      {/* 工作区（会话分组）列表弹层：点「工作区」行弹出（定位同对话列表弹层）；选中即设置并收起 */}
      {bindPick && bindWsOpen && (() => {
        const rightFits = bindPick.x + 300 + 260 <= window.innerWidth - 8
        const listLeft = rightFits ? bindPick.x + 300 : Math.max(8, bindPick.x - 268)
        const wsList = listWorkspaces()
        ...
        <div className="dsh-wt_menu dsh-wt_pop dsh-wt_bindListPop" style={{ position: 'fixed', left: listLeft, top: clamp(bindPick.y, 8, window.innerHeight - 348), width: 260, zIndex: 86 }}>
          <div className="dsh-wt_selectList dsh-wt_bindList">
            {wsList.length === 0 && <div className="dsh-wt_selectGroup">{t('bind.workspaceEmpty')}</div>}
            {wsList.map((w) => (
              <button
                key={w.id}
                type="button"
                className={'dsh-wt_selectItem' + (projects.workspaces[bindPick.id] === w.id ? ' dsh-wt_selectItemOn' : '')}
                title={w.path}
                onClick={() => setProjectWorkspace(bindPick.id, w.id)}
              >
                <span className="dsh-wt_selectItemTitle">{w.title || w.path || w.id}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* 应用内目录选择弹窗：native pickDirectory 不可用（browse 组合 / 远程访问）时的回退；
          面包屑 + 目录列表逐层浏览，路径输入框可手输回车跳转，「选择此文件夹」取输入框值 */}
      {dirPick && <div className="dsh-wt_popBackdrop" style={{ zIndex: 87 }} onClick={() => setDirPick(null)} />}
      {dirPick && (
        <div className="dsh-wt_menu dsh-wt_pop dsh-wt_dirPickPop" style={{ position: 'fixed', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 360, zIndex: 88 }}>
          <div className="dsh-wt_bindFolderRow">
            <span className="dsh-wt_bindFolderLabel">📁 {t('dirpick.title')}</span>
            <button type="button" className="dsh-wt_bindFolderChange" onClick={() => setDirPick(null)}>{t('confirm.cancel')}</button>
          </div>
          <div className="dsh-wt_dirPickCrumbs">
            {dirPick.crumbs.map((c, i) => (
              <Fragment key={c.path + ':' + i}>
                {i > 0 && <span className="dsh-wt_dirPickSep" aria-hidden>/</span>}
                <button type="button" className="dsh-wt_dirPickCrumb" title={c.path} onClick={() => dirPickLoad(c.path)}>{c.name}</button>
              </Fragment>
            ))}
          </div>
          <input
            className="dsh-wt_customPathInput dsh-wt_dirPickInput"
            value={dirPick.manual}
            placeholder={t('dirpick.manualPh')}
            spellCheck={false}
            onChange={(e) => setDirPick((s) => (s ? { ...s, manual: e.target.value } : s))}
            onKeyDown={(e) => { if (e.key === 'Enter' && dirPick.manual.trim()) dirPickLoad(dirPick.manual.trim()) }}
          />
          <div className="dsh-wt_dirPickNew">
            <input
              className="dsh-wt_customPathInput"
              value={dirPick.newName}
              placeholder={t('dirpick.newPh')}
              spellCheck={false}
              onChange={(e) => setDirPick((s) => (s ? { ...s, newName: e.target.value } : s))}
              onKeyDown={(e) => { if (e.key === 'Enter') dirPickCreate() }}
            />
            <button type="button" className="dsh-wt_bindFolderChange" disabled={!dirPick.newName.trim() || dirPick.loading} onClick={dirPickCreate}>
              ＋ {t('dirpick.newBtn')}
            </button>
          </div>
          <div className="dsh-wt_selectList dsh-wt_dirPickList">
            {dirPick.loading && <div className="dsh-wt_dirPickMsg">{t('dirpick.loading')}</div>}
            {!dirPick.loading && dirPick.error && <div className="dsh-wt_dirPickMsg dsh-wt_dirPickErr">{dirPick.error}</div>}
            {!dirPick.loading && !dirPick.error && dirPick.entries.length === 0 && <div className="dsh-wt_dirPickMsg">{t('dirpick.empty')}</div>}
            {!dirPick.loading && !dirPick.error && dirPick.entries.map((en) => (
              <button key={en.path} type="button" className="dsh-wt_selectItem" title={en.path} onClick={() => dirPickLoad(en.path)}>
                <span aria-hidden>📁</span>
                <span className="dsh-wt_selectItemTitle">{en.name}</span>
              </button>
            ))}
          </div>
          <button
            type="button"
            className="dsh-wt_customSend dsh-wt_dirPickOk"
            disabled={!dirPick.manual.trim()}
            onClick={() => { const p = dirPick.manual.trim(); if (!p) return; dirPick.apply(p); setDirPick(null) }}
          >
            {t('dirpick.confirm')}
          </button>
        </div>
      ...
      <div className="dsh-wt_projects" data-managing={viewOptionsOpen ? 'true' : undefined}>
        ...
        >
          ...
          <span
            className="dsh-wt_chatBtn"
            role="button"
            tabIndex={0}
            data-closed={rowChatClosed(CONSOLE_ID) ? 'true' : 'false'}
            data-tip={t(rowChatClosed(CONSOLE_ID) ? 'rowChat.show' : 'rowChat.hide')}
            aria-label={t(rowChatClosed(CONSOLE_ID) ? 'rowChat.show' : 'rowChat.hide')}
            onClick={(e) => { e.stopPropagation(); toggleRowChat(CONSOLE_ID, () => openConsole()) }}
          ><span className="dsh-wt_chatGlyph" aria-hidden /></span>
          <span
        ...
        {visibleLayouts.map((l) => (
          ...
          >
            ...
            <span
              ...
              role="button"
              tabIndex={0}
              data-closed={rowChatClosed(l.id) ? 'true' : 'false'}
              data-tip={t(rowChatClosed(l.id) ? 'rowChat.show' : 'rowChat.hide')}
              aria-label={t(rowChatClosed(l.id) ? 'rowChat.show' : 'rowChat.hide')}
              onClick={(e) => { e.stopPropagation(); toggleRowChat(l.id, () => { openSplit(l); reportUsed(l.id) }) }}
            ><span className="dsh-wt_chatGlyph" aria-hidden /></span>
            <span
              className={'dsh-wt_bindBtn'}
              role="button"
...
export function apply(ctx: any) {
  ...
  ctx.effect(() => {
    const over = (e: MouseEvent) => {
      ...
      const btn = t && t.closest ? (t.closest('.dsh-wt_bindBtn,.dsh-wt_chatBtn') as HTMLElement | null) : null
      if (btn) showBindTip(btn)
    }
    const out = (e: MouseEvent) => {
      const t = e.target as HTMLElement | null
      if (t && t.closest && t.closest('.dsh-wt_bindBtn,.dsh-wt_chatBtn')) hideBindTip()
    }
